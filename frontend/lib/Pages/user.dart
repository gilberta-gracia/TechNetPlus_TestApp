...
class _UsersState extends State<Users> {
  ...
  Future<void> showRole() async {
    ...
    if (response.statusCode == 200) {
      setState(() {
        ...
        // var rolesNames = responseData['rolesNames'];
        // getRole = rolesNames;
      });
    }
  }
  ...
  Future<void> deleteRole() async {
    ...
    final response = await http.post(
      Uri.parse('http://10.0.2.2:8000/api/users/delete_role'),
      headers: {'Content-Type': 'application/json'},
      body: jsonEncode(data),
    );
    if (response.statusCode == 200) {
  ...
  void createUser(BuildContext context) {
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          title: Text('Création', style: TextStyle(fontSize: 26)),
          content: SizedBox(
              height: 280,
              width: 300,
              child: Column(
  ...
  void updateUser(BuildContext context) {
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          title: Text('Modification', style: TextStyle(fontSize: 26)),
          content: SizedBox(
              height: 280,
              width: 300,
              child: Column(
  ...
  void Role(BuildContext context) {
    showDialog(
        ...
        builder: (BuildContext context) {
          return AlertDialog(
            title: Text('Rôle', style: TextStyle(fontSize: 26)),
            content: SizedBox(
              height: 280,
              width: 300,
...
Future<ConfirmAction?> _asyncConfirmDialog(BuildContext context) async {
  return showDialog<ConfirmAction>(
    ...
    builder: (BuildContext context) {
      return AlertDialog(
        title: Text('Suppression', style: TextStyle(fontSize: 26)),
...
void _getRole() async {
  ...
  // Récupérer la valeur des rôles en tant que chaîne JSON
  String roleNamesJson = prefs.getString('role') ?? "";

  if (roleNamesJson != null) {
    try {
      // Décoder la valeur JSON en une liste de noms de rôles
      List<String> roleNames = List<String>.from(jsonDecode(roleNamesJson));

      // Utiliser la liste de noms de rôles
